...
namespace TopSpeed.Server.Protocol
{
    internal static partial class PacketSerializer
    {
        ...
        public static byte[] WritePlayerLiveStart(PacketPlayerLiveStart live)
        {
            ...
            writer.WriteUInt32(live.PlayerId);
            writer.WriteByte(live.PlayerNumber);
            writer.WriteUInt32(live.StreamId);
            writer.WriteByte((byte)live.Codec);
            writer.WriteUInt16(live.SampleRate);
            writer.WriteByte(live.Channels);
            writer.WriteByte(live.FrameMs);
            return buffer;
        }

        public static byte[] WritePlayerLiveFrame(PacketPlayerLiveFrame live)
        {
            var bytes = live.Data ?? Array.Empty<byte>();
            if (bytes.Length == 0 || bytes.Length > ProtocolConstants.MaxLiveFrameBytes)
                throw new ArgumentOutOfRangeException(nameof(live), $"Live frame cannot exceed {ProtocolConstants.MaxLiveFrameBytes} bytes.");
            ...
            writer.WriteByte(live.PlayerNumber);
            writer.WriteUInt32(live.StreamId);
            writer.WriteUInt16(live.Sequence);
            writer.WriteUInt32(live.Timestamp);
        ...
        public static byte[] WritePlayerLiveStop(PacketPlayerLiveStop live)
        {
